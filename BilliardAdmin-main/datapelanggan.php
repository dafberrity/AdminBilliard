...
if (isset($_GET['delete'])) {
    $customerId = filter_var($_GET['delete'], FILTER_VALIDATE_INT);
    if ($customerId) {
...
// Fetch all customers
$stmt = $conn->query("SELECT id, name, phone_number, visit_count FROM customers ORDER BY name");
...
<head>
    ...
    <title>Data Pelanggan - Admin Billiard</title>
                    ...
                    <?php if (count($customers) > 0): ?>
                        <?php foreach ($customers as $index => $customer): ?>
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars($customer['visit_count']); ?>x</td>
                                <td>
                                    <a href="editpelanggan.php?id=<?php echo $customer['id']; ?>"><button>Edit</button></a>
                                    <a href="datapelanggan.php?delete=<?php echo $customer['id']; ?>" onclick="return confirm('Yakin ingin menghapus pelanggan ini?');"><button>Hapus</button></a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
