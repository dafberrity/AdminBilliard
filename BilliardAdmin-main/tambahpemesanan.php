...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($customer_name) || empty($table_number) || empty($start_time) || empty($duration) || empty($status)) {
        ...
        // Check for table availability (no overlapping bookings)
        $stmt = $conn->prepare("SELECT id FROM bookings WHERE table_number = ? AND status != 'Selesai' AND (
        ...
        if ($stmt->rowCount() > 0) {
            ...
            // Insert new booking
            $stmt = $conn->prepare("INSERT INTO bookings (customer_name, table_number, start_time, duration, status) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$customer_name, $table_number, $start_time, $duration, $status]);
            $_SESSION['success'] = "Pemesanan berhasil ditambahkan.";
